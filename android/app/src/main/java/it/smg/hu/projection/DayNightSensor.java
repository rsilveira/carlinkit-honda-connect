...
public class DayNightSensor implements ISensor, LocationListener, HondaConnectManager.HondaListener {
    ...
    private void initHondaIntegration() {
        ...
        HondaConnectManager cm = HondaConnectManager.instance();
        cm.addListener(this);
        Boolean isNight = cm.isNight();
        if (isNight != null) {
            currentState_ = isNight ? IS_NIGHT : IS_DAY;
            if (Log.isInfo()) Log.i(TAG, "Initialized Honda DayNight state: " + (isNight ? "NIGHT" : "DAY"));
        } else {
            currentState_ = IS_DAY;
            if (Log.isDebug()) Log.d(TAG, "Honda DayNight state unavailable on init, defaulting to DAY");
        }
    }
